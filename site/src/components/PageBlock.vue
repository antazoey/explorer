...
export default {
  ...
  methods: {
    ...
    async fetchBlock() {
      this.txUrl = process.env.VUE_APP_GATEWAY_URL
      let txs = await axios.post(this.txUrl, {
        block_height: parseInt(this.$route.params.block, 10),
        node_url: process.env.VUE_APP_RPC_URL
      })

      this.jsonUrl = `${this.blockchain.rpc}/block?height=${this.$route.params.block}`
      let json = await axios.get(this.jsonUrl)

      const block = json.data.result.block
      block.data.txs = txs.data
      this.block = block
    },
